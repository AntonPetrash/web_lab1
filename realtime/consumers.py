...
class DataConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if self.user and not self.user.is_anonymous:
            ...
            # Підписка на групу оновлень таймерів
            await self.channel_layer.group_add('timer_updates', self.channel_name)
            await self.accept()
    ...
    async def disconnect(self, close_code):
        if hasattr(self, 'user_id'):
            ...
            await self.channel_layer.group_discard('timer_updates', self.channel_name)
